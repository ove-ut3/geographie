...
generer_data <- function() {
  ...
  cp_ville_commune <- dplyr::select(ptt, code_postal, lib_commune1 = nom_localite, code_commune) %>%
    ...
    dplyr::mutate(lib_commune1 = caractr::sans_ponctuation(lib_commune1) %>% caractr::str_remove_accent(),
                  lib_commune2 = caractr::sans_ponctuation(lib_commune2) %>% caractr::str_remove_accent(),
                  lib_commune3 = caractr::sans_ponctuation(lib_commune3) %>% caractr::str_remove_accent()) %>%
