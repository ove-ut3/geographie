...
geocoder_adresse <- function(adresse, service, nettoyer_adresse = TRUE, timeout = 10) {
  ...
  if (service == "adresse.data.gouv.fr") {
    base_geocodage <- impexp::rdata_import(paste0(racine_packages, "geographie/data/geocodage_data_gouv.RData"))
  } else if (service == "googlemaps") base_geocodage <- impexp::rdata_import(paste0(racine_packages, "geographie/data/geocodage_google.RData"))
...
geocoder_adresse_data_gouv <- function(adresse, timeout = 10) {
  ...
  geocodage_data_gouv <- dplyr::filter(geocoder, purrr::map_lgl(erreur, is.null)) %>%
    dplyr::select(-erreur) %>%
    dplyr::bind_rows(impexp::rdata_import(paste0(racine_packages, "geographie/data/geocodage_data_gouv.RData"))) %>%
...
geocoder_adresse_google <- function(adresse, timeout = 10) {
  ...
  if (test_quota == "OVER_QUERY_LIMIT") {
    ...
    geocoder <- left_join(geocoder_init,
                          impexp::rdata_import(paste0(racine_packages, "geographie/data/geocodage_google.RData")),
  ...
  geocodage_google <- dplyr::filter(geocoder, !nchar(erreur) != 0) %>%
    ...
    dplyr::bind_rows(impexp::rdata_import(paste0(racine_packages, "geographie/data/geocodage_google.RData"))) %>%
    dplyr::arrange(adresse)
